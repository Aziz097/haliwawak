...
export default function AdminSidebar({ user }: { user?: { name: string; role: string } }) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 bottom-0 ${collapsed ? 'w-[72px]' : 'w-[260px]'} bg-kiosk-ink text-white flex flex-col z-50 transition-all duration-300`}
    >
      <div
        className={`p-5 border-b border-white/10 flex items-center ${collapsed ? 'justify-center' : 'gap-3'}`}
      >
        <div className="w-9 h-9 rounded-full bg-kiosk-orange-600 flex items-center justify-center shrink-0">
          <Bug className="w-5 h-5 text-white" />
        </div>
        {!collapsed && (
          <span className="font-heading font-bold text-lg tracking-tight">CMS Pugung</span>
        )}
      </div>
        ...
        {GROUPS.map((group) => {
          ...
          return (
            <div key={group} className="mb-4">
              {!collapsed && (
                <p className="px-5 mb-2 text-[10px] font-bold text-white/40 uppercase tracking-widest">
                  {group}
                </p>
              )}
